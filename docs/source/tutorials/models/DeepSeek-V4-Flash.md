...
You can using our official docker image to run `DeepSeek-V4` directly. Currently, `DeepSeek-V4` is integrated in image `v0.18.0rc?`.
...
export IMAGE=quay.io/ascend/vllm-ascend:v0.18.0rc?
...
export IMAGE=quay.io/ascend/vllm-ascend:v0.18.0rc?-a3
export NAME=vllm-ascend
docker run --rm \
    --name $NAME \
    --net=host \
...
:::{note}
Please use the v0.18.0rc? code to install vllm-ascend.
:::
