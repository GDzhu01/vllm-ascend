...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device, pass_hidden_states_to_model: bool, runner=None):
        ...
        if (
            self.method == "mtp"
            and getattr(draft_hf_config, "model_type", None) == "deepseek_v4"
    ...
    def _propose(
        ...
        for attn_group in self.draft_attn_groups:
            ...
            attn_metadata = self._freeze_draft_step_attn_metadata(
                attn_metadata)
    ...
    def _run_merged_draft(
        ...
        if self.pcp_size > 1:
            ...
            if self.method == "mtp":
                if last_hidden_states.shape == hidden_states.shape:
                    ...
                    last_hidden_states = get_pcp_group().all_gather(
        ...
        for draft_step in range(self.num_speculative_tokens - 1):
            ...
            can_copy_hidden_states = (
                self.method != "mtp"
                or hidden_states.view(batch_size, -1).shape[1:] == self.hidden_states.shape[1:]
            )
            if can_copy_hidden_states:
    ...
    def set_inputs_first_pass(
        ...
        if not self.needs_extra_input_slots:
            ...
            target_hidden_states_flat = target_hidden_states.view(num_tokens, -1)
            if (self.method == "mtp"
                    and target_hidden_states_flat.shape[1:] != self.hidden_states.shape[1:]):
                self.mtp_initial_hidden_states = target_hidden_states
            else:
                self.mtp_initial_hidden_states = None
                self.hidden_states[:num_tokens] = target_hidden_states_flat
    ...
    def model_returns_tuple(self) -> bool:
        return self.method not in ("mtp", "draft_model", "dflash") or getattr(
    ...
    def attn_update_stack_num_spec_norm(
        ...
        if advance_metadata:
            ...
            seq_len_delta_cpu = seq_len_delta.cpu() if isinstance(seq_len_delta, torch.Tensor) else seq_len_delta
            if common_attn_metadata.seq_lens_cpu is not None:
            ...
            if common_attn_metadata._seq_lens_cpu is not None:
                common_attn_metadata._seq_lens_cpu[:batch_size] = (
                    common_attn_metadata._seq_lens_cpu[:batch_size] + seq_len_delta_cpu)
